...
def get_iterators(args, batch_size):
    ...
    FORM_TO_CHAR = data.Field(tokenize=tokeniser, batch_first=True)
    CHAR = data.NestedField(FORM_TO_CHAR, tokenize=list)
    ...
    train, dev, test = data.TabularDataset.splits(path=".tmp", train='train.csv', validation='dev.csv', test='test.csv',
                                                  format="csv", fields=[('id', ID), ('form', FORM), ('char', CHAR),
                                                                        ('lemma', LEMMA), ('upos', UPOS),
                                                                        ('xpos', XPOS), ('feats', FEATS),
                                                                        ('head', HEAD), ('deprel', DEPREL),
                                                                        ('deps', DEPS), ('misc', MISC)])

    fields = [ID, FORM, CHAR, LEMMA, UPOS, XPOS, FEATS, HEAD, DEPREL, DEPS, MISC]
