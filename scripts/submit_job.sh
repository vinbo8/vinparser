...
git pull
echo "$root/venv/bin/python $root/Runner.py --tag --parse --use_cuda --code-switch \
--train $root/data/codeswitch/en-hi/en-hi-train-append.conllu \
--dev $root/data/codeswitch/en-hi/en-hi-dev.conllu --test $root/data/codeswitch/en-hi-test.conllu \
