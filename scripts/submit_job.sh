...
git checkout analyzer2
git pull
echo "$root/venv/bin/python -u $root/Runner.py --morph --use_cuda \
...
qsub -e "$root/results/err" -o "$root/results/$1" -N "$1" -q gpu-ms.q@dll[256] -l gpu=1,gpu_cc_min3.5=1,gpu_ram=6G parse.sh
