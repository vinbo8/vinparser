...
class Tagger(torch.nn.Module):
    ...
    def forward(self, forms, pack):
        ...
        packed = torch.nn.utils.rnn.pack_padded_sequence(form_embeds, pack.data.tolist(), batch_first=True)
    ...
    def train_(self, epoch, train_loader):
        ...
        for i, batch in enumerate(train_loader):
            x_forms, y_tags, x_chars, mask, pack, y_heads, y_deprels = process_batch(batch)
            ...
            y_tags = y_tags.contiguous().view(batch_size * longest_sentence_in_batch)

            train_loss = self.criterion(y_pred, y_tags)
    ...
    def evaluate_(self, test_loader):
        ...
        for i, batch in enumerate(test_loader):
            ...
            try:
                correct += ((y_tags == y_pred) * mask.type(torch.ByteTensor)).nonzero().size(0)
            except RuntimeError:
                pass
...
def main():
    ...
    _, dev_loader = build_data(args.train, BATCH_SIZE)
    _, test_loader = build_data(args.test, BATCH_SIZE, conll)

    tagger = Tagger(conll.sizes, args)
